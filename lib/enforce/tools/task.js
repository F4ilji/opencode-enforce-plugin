  ...
  return {
    ...
    async execute(args) {
      ...
      if (!st.plan || typeof st.plan !== "object" || !("acceptance_criteria" in st.plan)) {
        st.plan = { acceptance_criteria: [], subgoals: [] };
      }
      st.plan.acceptance_criteria = acceptance_criteria;
      ...
      st.step_index = st.step_index || 0;
      st.last_observation = `Plan created with ${affected_files.length} files, risk: ${risk_level}`;
      st.last_activity = new Date().toISOString();
      ...
      // Auto-approve low-risk plans — trivial task, no user gate needed
      if (risk_level === "low") {
        ...
        const approvalWritten = writeFileContent(
          ...
          JSON.stringify(autoApproval, null, 2),
        );
        if (approvalWritten) {
          const stAfter = readState(directory);
          stAfter.plan_approved_at = now;
          stAfter.phase = "implementing";
          stAfter.last_observation = `Plan auto-approved (low risk). Ready to implement.`;
          stAfter.last_activity = new Date().toISOString();
          writeState(directory, stAfter);
          logAudit(directory, { event: "plan_auto_approved", task_id, risk_level });
          ...
          return {
            ...
            warning,
            auto_approved: true,
            next: "Low-risk plan auto-approved. Proceed to implementation.",
          };
        }
      }
...
export function approvePlanTool(context) {
  return {
    ...
    async execute(args) {
      ...
      st.last_observation = `Plan approved. Risk: ${plan.risk_level}. Ready to implement.`;
      st.last_activity = new Date().toISOString();
      if (!writeState(directory, st)) {
        logAudit(directory, {
          event: "state_write_failed",
  ...
  return {
    ...
    async execute(args) {
      ...
      if (highRiskFiles.length > 0) {
        const approvalFile = join(approvalsDir, `${task_id}.plan_approved.json`);
        if (!existsSync(approvalFile)) {
          return {
            status: "error",
            reason: `High-risk changes require plan approval`,
            high_risk_files: highRiskFiles,
            instruction: `Call approve_plan(${task_id}) to approve the plan before completing this task`,
          };
        }
      }
      ...
      st.task_files = [];
      st.plan_warning_sent = false;
      st.last_observation = `Task ${task_id} completed. ${summary}`;
      st.last_activity = new Date().toISOString();
      if (st.plan?.subgoals) {
        st.plan.subgoals = st.plan.subgoals.map(sg => ({ ...sg, status: "completed" }));
      }
      if (st.working_context) {
        st.working_context.known_blockers = [];
        st.working_context.active_hypotheses = [];
      }
