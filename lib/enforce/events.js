    ...
    if (event.type === "session.idle") {
      if (context.winEdited.size > 0) {
        ...
        if (!preflight.passed) {
          ...
          const msg = `❌ Pre-flight failed at step: ${preflight.step}. Docker restart blocked.\n\n${trimOutput(preflight.output, 30)}\n\n(fix the errors and save the file — the check re-runs automatically after your next edit)`;
        ...
        for (const svc of neededServices) {
          ...
          try {
            spawnSync("docker", ["compose", "restart", svc], {
              cwd: directory,
              timeout: 60000,
            });
            const st = readState(directory);
