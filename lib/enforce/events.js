...
export function createEventHandler(client, context) {
  ...
  return async ({ event }) => {
    ...
    if (event.type === "file.edited" || (event.type === "file.watcher.updated" && p.event === "add")) {
      ...
      // FSM guard: warn agent on EVERY file edit without active task
      const hasActiveTask = st.task_id && st.phase !== "completed" && st.phase !== "aborted";
      if (!hasActiveTask) {
        context.taskViolationCount++;
        const count = context.taskViolationCount;
        
        let message;
        if (count <= 2) {
          message = `⚠️ No active task detected (violation #${count}). File edits without task tracking violate FSM lifecycle.\n\nRequired workflow:\n1. begin_task(task_id, description, files_whitelist, priority)\n2. create_plan(task_id, affected_files, risk_level, acceptance_criteria)\n3. [approve_plan if medium/high risk]\n4. validate_changes(task_id)\n5. commit_task(task_id, type, scope, summary)\n\nCall begin_task() now to initialize tracking.`;
        } else if (count <= 5) {
          message = `🚫 BLOCKED: No active task (violation #${count}). You MUST call begin_task() before editing files. Continuing without task tracking is a protocol violation.\n\nRequired: begin_task(task_id, description, files_whitelist, priority)`;
        } else {
          message = `🛑 CRITICAL: ${count} violations without active task. STOP all file edits immediately. Call begin_task() NOW or abort the session. This is your final warning.`;
        }
        
        await sendMessage(context.sessionId, message);
      }
