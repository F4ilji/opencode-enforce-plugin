    ...
    if (event.type === "file.edited") {
      ...
      if (f) {
        ...
        if (!isServiceFile(rel, directory)) {
          ...
          const hasActiveTask = st.task_id && st.phase !== "completed" && st.phase !== "done";
          const hasApprovedPlan = !!st.plan_approved_at;
          const taskFileCount = Array.isArray(st.task_files) ? st.task_files.length : 0;

          if (!hasActiveTask) {
            if (taskFileCount === 0) {
              logAudit(directory, {
                event: "protocol_violation",
                violation: "edit_without_task",
                file: rel,
                severity: "critical",
              });
              incrementMetric(
              ...
              await sendMessage(
                context.sessionId,
                `🚫 CRITICAL: Edit detected without active task.\n` +
                  `File: \`${rel}\`\n` +
                  `This violates the enforce protocol.\n\n` +
                  `STOP editing. You MUST:\n` +
                  `1. Call create_task(task_id, description, priority, acceptance_criteria)\n` +
                  `2. Call create_plan(task_id, affected_files, risk_level, acceptance_criteria)\n` +
                  `3. Wait for user approval\n` +
                  `4. Call approve_plan(task_id)\n` +
                  `5. THEN edit files`,
              );
              return;
            }

            if (!st.plan_warning_sent) {
              ...
              await sendMessage(
                ...
                `🚫 CRITICAL: Edit detected without active task.\n` +
                  `File: \`${rel}\`\n` +
                  `${taskFileCount} file(s) already tracked without a task.\n\n` +
                  `STOP editing. You MUST:\n` +
                  `1. Call create_task(task_id, description, priority, acceptance_criteria)\n` +
                  `2. Call create_plan(task_id, affected_files, risk_level, acceptance_criteria)\n` +
                  `3. Wait for user approval\n` +
          ...
          if (hasActiveTask && !hasApprovedPlan && taskFileCount >= 1) {
            if (!st.plan_warning_sent) {
              st.plan_warning_sent = true;
              logAudit(directory, {
                event: "protocol_violation",
                violation: "edit_without_approved_plan",
                file: rel,
                task_id: st.task_id,
                severity: "high",
              });
              ...
              await sendMessage(
                context.sessionId,
                  `🚫 PROTOCOL VIOLATION: Edit detected without approved plan.\n` +
                  `File: \`${rel}\`\n` +
                  `Task: ${st.task_id}\n\n` +
                  `STOP editing. You MUST:\n` +
                  `1. Call create_plan(${st.task_id}, [affected_files], risk_level, criteria)\n` +
                  `2. Wait for user approval\n` +
                  `3. Call approve_plan(${st.task_id})\n` +
                  `4. THEN continue editing`,
              );
            }
          }
