...
export function migratePhase0(directory) {
  ...
  if (!existsSync(stateFile)) {
    ...
    if (!state.plan || typeof state.plan !== "object" || !("acceptance_criteria" in state.plan)) {
      state.plan = {
        acceptance_criteria: state.plan?.acceptance_criteria || [],
        subgoals: state.plan?.subgoals || [],
      };
      dirty = true;
    }
    if (!state.working_context || typeof state.working_context !== "object") {
      state.working_context = {
        modified_files: state.task_files || [],
        active_hypotheses: [],
        known_blockers: [],
      };
      dirty = true;
