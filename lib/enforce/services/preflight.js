...
async function runLocal(innerCmd, timeoutMs = 60000) {
  const opts = { encoding: "utf8", timeout: timeoutMs, maxBuffer: 10 * 1024 * 1024 };
  return await execFileAsync("sh", ["-c", innerCmd], opts);
}
...
export async function runInContainer(
  ...
  const cfg = getConfig(directory);
  if (cfg.local) {
    return await runLocal(innerCmd, timeoutMs);
  }
  try {
    const opts = { cwd: directory, encoding: "utf8", timeout: timeoutMs, maxBuffer: 10 * 1024 * 1024 };
...
function infraFailure(res, timeoutMs, isLocal = false) {
  if (res.error) {
    ...
    if (isLocal) {
      return {
        kind: "infra",
        output: `Local execution failed: ${res.error.message}`,
      };
    }
    return {
      kind: "infra",
  ...
  if (!isLocal && isDockerInfraError(res)) {
    return {
      kind: "infra",
...
export async function runPreflight(directory, batch = []) {
  ...
  const steps = [];
  {
    const cmd = cfg.preflight.compileall_cmd;
    if (!cmd || cmd.trim() === "") {
      steps.push({
        name: "compileall",
        status: "skipped",
        reason: "compileall_cmd not configured",
      });
    } else {
      ...
      const infra = infraFailure(res, cfg.preflight.timeouts.compileall, cfg.local);
      if (infra)
        return {
          passed: false,
          step: "compileall",
  ...
  if (lintableFiles.length === 0 || !cfg.preflight.linter_cmd || cfg.preflight.linter_cmd.trim() === "") {
    steps.push({
      name: "linter",
      status: "skipped",
      reason: !cfg.preflight.linter_cmd || cfg.preflight.linter_cmd.trim() === "" ? "linter_cmd not configured" : "no lintable files in change batch",
    });
  } else {
    ...
    for (const c of lintableFiles) {
      ...
      const infra = infraFailure(res, cfg.preflight.timeouts.ruff, cfg.local);
      if (infra)
        return {
          passed: false,
          step: "linter",
  ...
  if (cfg.preflight.test_cmd && cfg.preflight.test_cmd.trim() !== "") {
    const res = await runInContainer(
      directory,
      cfg.preflight.test_cmd,
      cfg.preflight.timeouts.pytest,
    );
    const infra = infraFailure(res, cfg.preflight.timeouts.pytest, cfg.local);
    if (infra)
      return {
        passed: false,
        step: "test",
    ...
    steps.push({
      name: "test",
      status: "skipped",
      reason: "test_cmd not configured",
    });
  }
...
export async function waitForHealth(directory, service, timeoutMs = 30000) {
  const cfg = getConfig(directory);
  if (cfg.local) {
    return { healthy: true, assumed: true };
  }
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    try {
      const idRes = await execFileAsync("docker", ["compose", "ps", "-q", service], {
        cwd: directory,
        encoding: "utf8",
        timeout: 5000,
      });
