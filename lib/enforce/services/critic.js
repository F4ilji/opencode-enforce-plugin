...
function parseCriticResponse(content) {
  if (!content || typeof content !== "string") {
    return { verdict: "unknown", feedback: ["Empty response from critic"], raw_response: content };
  }
  ...
  try {
    ...
    return {
      ...
      feedback: Array.isArray(parsed.feedback) ? parsed.feedback : [String(parsed.feedback || "")],
      raw_response: content,
    };
  } catch {
    const verdictMatch = content.match(/"verdict"\s*:\s*"(approved|changes_requested)"/i)
      || content.match(/verdict:\s*(approved|changes_requested)/i);
    return {
